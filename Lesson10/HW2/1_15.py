...
label2 = Label(
    text='Введіть шлях, за яким файл буде збережений на пристрій')  # віджет призначений для відображення напису
...
def save_file():  # створюємо функцію, аргументи вона приймати не буде, вона буде працювати з глобальними змінними entry і label
    ...
    # print(r)
    if entry2.get() != None:  # якщо користувач ввів директорію, зберігає в директорію
        a = entry2.get()  # a = шлях до директорії
    else:
        a = None

    if entry3.get() != None:  # якщо користувач ввів директорію, зберігає в директорію
        b = entry3.get()  # b = назва файлу
    else:
        b = s.split("/")[-1]
        # print(b)
